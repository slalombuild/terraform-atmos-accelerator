...
This component is responsible for creating CloudWatch Events rules and according
targets.
  ...
  cloudwatch:
    vars:
      cloudwatch_event_rule_pattern:
          ...
          detail:
            service: "EC2"
            eventYypeCategory: ["issue"]
```

Perform the following commands on the root folder:

- `atmos terraform plan cloudwatch -s {namespace}-{environment}` to see the
  infrastructure execution plan
- `atmos terraform apply cloudwatch -s {namespace}-{environment}` to execute the
  action proposed at terraform plan
- `atmos terraform destroy cloudwatch -s {namespace}-{environment}` to destroy
  all remote objects
...
| <a name="module_sns"></a> [sns](#module\_sns) | cloudposse/sns-topic/aws | 0.21.0 |
...
| <a name="input_sns_topic_allowed_aws_services_for_sns_published"></a> [sns\_topic\_allowed\_aws\_services\_for\_sns\_published](#input\_sns\_topic\_allowed\_aws\_services\_for\_sns\_published) | AWS services that will have permission to publish to SNS topic. Used when no external json policy is used. | `list(string)` | <pre>[<br>  "cloudwatch.amazonaws.com"<br>]</pre> | no |
| <a name="input_sns_topic_subscribers"></a> [sns\_topic\_subscribers](#input\_sns\_topic\_subscribers) | Required configuration for subscibres to SNS topic. | <pre>map(object({<br>    protocol = string<br>    # The protocol to use. The possible values for this are: sqs, sms, lambda, application. (http or https are partially supported, see below) (email is an option but is unsupported, see below).<br>    endpoint = string<br>    # The endpoint to send data to, the contents will vary with the protocol. (see below for more information)<br>    endpoint_auto_confirms = bool<br>    # Boolean indicating whether the end point is capable of auto confirming subscription e.g., PagerDuty (default is false)<br>  }))</pre> | `{}` | no |
