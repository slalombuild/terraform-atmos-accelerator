...
# Provision AWS Access Analyzer
module "access_analyzer_label" {
  source  = "cloudposse/label/null"
  version = "0.25.0"

  attributes = ["iam", "access", "analyzer"]
  context    = module.this.context
}

# Enable AWS Access Analyzer for the entire organization
resource "aws_accessanalyzer_analyzer" "organization_analyzer" {
  count = var.access_analyzer_enabled ? 1 : 0

  analyzer_name = module.access_analyzer_label.id
  type          = "ORGANIZATION"
}