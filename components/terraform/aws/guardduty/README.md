...
This component enables AWS GuardDuty in one region of one account and optionally
sets up an SNS topic to receive notifications of its findings.
...
- `atmos terraform plan guardduty -s {namespace}-{environment}` to see the
  infrastructure execution plan
- `atmos terraform apply guardduty -s {namespace}-{environment}` to execute the
  action proposed at terraform plan
- `atmos terraform destroy guardduty -s {namespace}-{environment}` to destroy
  all remote objects
