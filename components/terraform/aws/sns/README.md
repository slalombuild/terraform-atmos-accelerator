...
This component is responsible for provisioning SNS topic.
...
- `atmos terraform plan sns -s {namespace}-{environment}` to see the
  infrastructure execution plan
- `atmos terraform apply sns -s {namespace}-{environment}` to execute the action
  proposed at terraform plan
- `atmos terraform destroy sns -s {namespace}-{environment}` to destroy all
  remote objects
...
| Name | Description |
|------|-------------|
| <a name="output_sns"></a> [sns](#output\_sns) | SNS |
<!-- END-TERRAFORM-DOCS -->
