...
This component enables AWS Config and optionally sets up an SNS topic to receive
...
- `atmos terraform plan config -s {namespace}-{environment}` to see the
  infrastructure execution plan
- `atmos terraform apply config -s {namespace}-{environment}` to execute the
  action proposed at terraform plan
- `atmos terraform destroy config -s {namespace}-{environment}` to destroy all
  remote objects
