...
This component is responsible for provisioning the primary DNS zones into an AWS
account. By convention, we typically provision the primary DNS zones in the
`dns/automation` account. The primary account for branded zones (e.g.
`accelerator.slalom.com`), however, would be in the
`prod.accelerator.slalom.com` account, while staging zone (e.g.
`accelerator.cudaops.qa`) might be in the `staging` account.

The zones from the primary DNS zone are then expected to be delegated to other
accounts via [the `dns-delegated` component](../dns-delegated). Additionally,
external records can be created on the primary DNS zones via the `record_config`
variable.
...
Here's an example snippet for how to use this component. This component should
only be applied once as the DNS zones it creates are global. This is typically
done via the DNS stack (e.g. `gbl-dns.yaml`).
...
| <a name="input_soa_record_ttl"></a> [soa\_record\_ttl](#input\_soa\_record\_ttl) | TTL of the SOA record for the domain | `number` | `60` | no |
