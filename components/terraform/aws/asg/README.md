...
This component is responsible to provision Auto Scaling Group and Launch
Template on AWS.
...
file `stack/catalog/asg/defaults.yaml`

```yaml
# stacks/catalog/asg/defaults.yaml
components:
  terraform:
    asg/defaults:
      metadata:
      ...
      vars:
        ...
        # to enable or disable imdsv2 metadata auth
...
- `atmos terraform plan asg -s {namespace}-{environment}` to see the
  infrastructure execution plan
- `atmos terraform apply asg -s {namespace}-{environment}` to execute the action
  proposed at terraform plan
- `atmos terraform destroy asg -s {namespace}-{environment}` to destroy all
  remote objects
...
| [aws_security_group_rule.allow_http_ingress](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/security_group_rule) | resource |
...
| [aws_lb_target_group.alb](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/data-sources/lb_target_group) | data source |
...
| <a name="input_alb_name"></a> [alb\_name](#input\_alb\_name) | Application Load Balancer Name | `string` | `""` | no |
...
| <a name="input_block_device_mappings"></a> [block\_device\_mappings](#input\_block\_device\_mappings) | Specify volumes to attach to the instance besides the volumes specified by the AMI | <pre>list(object({<br>    device_name  = string<br>    no_device    = bool<br>    virtual_name = string<br>    ebs = object({<br>      delete_on_termination = bool<br>      encrypted             = bool<br>      iops                  = number<br>      kms_key_id            = string<br>      snapshot_id           = string<br>      volume_size           = number<br>      volume_type           = string<br>    })<br>  }))</pre> | <pre>[<br>  {<br>    "device_name": "/dev/sdb",<br>    "ebs": {<br>      "delete_on_termination": true,<br>      "encrypted": true,<br>      "iops": null,<br>      "kms_key_id": null,<br>      "snapshot_id": null,<br>      "volume_size": 8,<br>      "volume_type": "gp2"<br>    },<br>    "no_device": null,<br>    "virtual_name": null<br>  }<br>]</pre> | no |
...
| <a name="input_service_port"></a> [service\_port](#input\_service\_port) | HTTP port | `number` | `80` | no |
| <a name="input_ssh_ingress_cidrs"></a> [ssh\_ingress\_cidrs](#input\_ssh\_ingress\_cidrs) | List of CIDR blocks to allow SSH ingress | `list(string)` | `[]` | no |
...
| <a name="input_userdata_file"></a> [userdata\_file](#input\_userdata\_file) | The userdata script name from component/asg/userdata to use for the instances | `string` | `"accelerator_user_data.sh"` | no |
