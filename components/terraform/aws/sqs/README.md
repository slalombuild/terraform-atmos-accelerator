...
- `atmos terraform plan sqs -s {namespace}-{environment}` to see the
  infrastructure execution plan
- `atmos terraform apply sqs -s {namespace}-{environment}` to execute the action
  proposed at terraform plan
- `atmos terraform destroy sqs -s {namespace}-{environment}` to destroy all
  remote objects
